...
class LevelGenerator {
  ...
  static const _shapeLevels = <int, String>{
    ...
    93: 'flower', 99: 'peach',
    // L100+ shapes use the gravity packer (see _gravityShapes / _packGravity).
    104: 'shield',
  };

  /// Shapes generated with [_packGravity] (100% fill, solvable by
  /// construction, single-digit-ms generation) instead of RC packing.
  ...
  /// Cells left unfilled by [_packGravity] (dropped orphan singletons).
  /// Zero on almost every seed; genLevel retries when non-zero.
  int _gravityHoles = 0;

  List<Arrow> _packGravity(int seed) {
    _gravityHoles = 0;
    final rng = SeededRandom(seed);
    ...
    for (var y = 0; y <= rows; y++) {
      for (var x = 0; x <= cols; x++) {
        if (_inB(x, y)) mask.add(cellKey(x, y));
      }
    }
    final arrows = <Arrow>[];
  ...
  /// tried first. Every accepted state is verified with [greedySolvable];
  ...
  void _mixByFlipping(SeededRandom rng, List<Arrow> arrows) {
    ...
    for (var s = 0;
      ...
      if (!greedySolvable(arrows)) {
        ...
        for (var b = 0; b < batch.length && checks < maxChecks; b++) {
          ...
          arrows[i] = flip(arrows[i]);
          checks++;
          if (!greedySolvable(arrows)) arrows[i] = saved[b];
        }
      }
    }
  }
  ...
  /// leaving a 1-cell hole (counted in [_gravityHoles] → seed retry).
  ...
  GeneratedLevel genLevel(int level, {bool daily = false}) {
    ...
    if (_shapeMask != null && _gravityShapes.contains(shapeName)) {
      ...
      for (var att = 0; att < 6; att++) {
        final arr = _packGravity((seed + att * 7919) & 0xFFFFFFFF);
        if (!greedySolvable(arr)) continue;
        if (_gravityHoles < bestHoles) {
          bestHoles = _gravityHoles;
          best = arr;
        }
        if (bestHoles == 0) break;
      }
      best ??= _packGravity(seed); // unreachable in practice
      _mixByFlipping(SeededRandom(seed ^ 0x51F1), best);
